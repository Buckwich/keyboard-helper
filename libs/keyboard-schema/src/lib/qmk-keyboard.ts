...
type McuPinArray = ('NO_PIN' | string | number | null)[];
/**
 * @minItems 1
 */
type KeycodeDeclArray = [KeycodeDecl, ...KeycodeDecl[]];
/**
 * @minItems 2
 * @maxItems 2
 */
type McuPinArray1 = ['NO_PIN' | string | number | null, 'NO_PIN' | string | number | null];

interface KeyboardInformation {
  ...
  backlight?: {
    driver?: 'pwm' | 'software' | 'timer' | 'custom';
    default?: {
      on?: boolean;
      breathing?: boolean;
      brightness?: number;
  ...
  split?: {
    ...
    transport?: {
      protocol?: 'custom' | 'i2c' | 'serial' | 'serial_usart';
      sync?: {
        activity?: boolean;
        detected_os?: boolean;
        haptic?: boolean;
...
interface DipSwitchConfig1 {
  pins?: McuPinArray;
  [k: string]: unknown;
}
interface EncoderConfig1 {
  rotary?: {
...
export type { KeyboardInformation as QMKKeyboard };
